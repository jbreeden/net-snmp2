...
=== IMPORTANT
These environment variables, if used, must be set before requiring this gem. Net-snmp is performing its read of the mib when the library is loaded (that is, for this gem, when the Wrapper.rb file is included).
...
An SNMPv3 synchronous AuthPriv (encrypted) SNMP-GET
    ...
    Net::SNMP::Session.open(
          ...
          :community => "demopublic", :version => "3",
